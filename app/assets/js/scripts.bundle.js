...
document.querySelectorAll(".input-password-show").forEach((element) => {
...
document.querySelectorAll(".input-floting-label").forEach((element) => {
  element.querySelector("input").addEventListener("focus", (e) => {
    if (element.querySelector("label")) {
      element.querySelector("label").classList.add("floting");
    }
  });
  element.querySelector("input").addEventListener("blur", (e) => {
    if (!Boolean(e.target.value)) {
      element.querySelector("label").classList.remove("floting");
    }
  });
...
document.querySelectorAll("input, textarea, select").forEach((element) => {
  element.error = (params = {}) => {
    ...
    if (params.msg) {
      ...
      msgError.innerText = params.msg;
      msgError.id = "fv-" + element.id + "-error";
      element.parentNode.appendChild(msgError);
    }
  };
...
document.querySelectorAll(".select-img-input").forEach((btn) => {
    btn.addEventListener('click', () => {
        let inputFile = document.querySelector(btn.getAttribute('data-input-file'))
        inputFile.addEventListener("change", () => {
            if (inputFile.files && inputFile.files[0]) {
                const reader = new FileReader();
        
                // Cuando se cargue la imagen, actualiza el src de la imagen
